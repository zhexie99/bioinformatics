...
for PATH_TO_SAMPLE in "$PATH_TO_TUBER_SAMPLE"/Sample_24; do
    ...
    $PATH_TO_HISAT --dta-cufflinks --rna-strandness FR --rg-id H0NDCADXX.2 --rg PU:H0NDCADXX.2.1 --rg SM:${SAMPLE} --rg PL:ILLUMINA --rg LB:${LIBRARY} -p 8 -q -x $PATH_TO_ATLANTIC_INDEX -1 $PATH_TO_TRIMMED_READ1 -2 $PATH_TO_TRIMMED_READ2 -S sample${SAMPLE}.sam;

    samtools sort -o sample${SAMPLE}.sorted.bam sample${SAMPLE}.sam;
    ...
    cufflinks -g $PATH_TO_GTF --frag-bias-correct $PATH_TO_GENOME --multi-read-correct -o $OUTDIR sample${SAMPLE}.sorted.marked.fixed.bam;
